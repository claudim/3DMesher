...
void External_and_onBoundary_remover::removeBlocks(LCC_3 &lcc, const Polyhedron &polyhedron) {
    if(!lcc.is_empty())
    {
        for (LCC_3::One_dart_per_cell_range<3, 3>::iterator lcc_cells_iterator = lcc.one_dart_per_cell<3, 3>().begin(),
            ...
            if (this->is_block_to_be_removed(lcc, lcc_cells_iterator, polyhedron)) {
...
bool External_and_onBoundary_remover::is_block_to_be_removed(const LCC_3& lcc, const Dart_handle& block, const Polyhedron& polyhedron){
    bool toRemove = false;
    if(!lcc.is_empty()) {
