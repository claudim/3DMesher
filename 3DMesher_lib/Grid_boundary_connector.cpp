...
//    Tree tree(faces(polyhedron).first, faces(polyhedron).second, polyhedron);
        ...
        for (LCC_3::One_dart_per_cell_range<3, 3>::iterator block_iterator = lcc.one_dart_per_cell<3, 3>().begin(),
                     end_iterator = lcc.one_dart_per_cell<3, 3>().end();
             block_iterator != end_iterator; ++block_iterator) {
            for (LCC_3::One_dart_per_incident_cell_range<0, 3>::iterator vertex_iterator = lcc.one_dart_per_incident_cell<0, 3>(
                    block_iterator).begin(),
                         end_it = lcc.one_dart_per_incident_cell<0, 3>(block_iterator).end();
                 vertex_iterator != end_it; ++vertex_iterator) {
                // quanti_cicli ++;
