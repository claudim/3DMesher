...
std::vector<Dart_const_handle> External_facet_finder::find(const LCC_3 & mesh){
    std::vector<Dart_const_handle> externalFacets = std::vector<Dart_const_handle>();
    if(!mesh.is_empty())
    {
        for(LCC_3::One_dart_per_cell_const_range<2,3>::const_iterator cell_facet_iterator = mesh.one_dart_per_cell<2,3>().begin(),
                    cell_facet_end_iterator = mesh.one_dart_per_cell<2, 3>().end();
            cell_facet_iterator != cell_facet_end_iterator; ++cell_facet_iterator){
            Dart_const_handle near_cell_facet_dart_handle = mesh.beta(cell_facet_iterator, 3);
            if (is_near_cell_facet_null(mesh, near_cell_facet_dart_handle))
            {
